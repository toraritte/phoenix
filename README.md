...
`Phoenix.Router.Helpers` generates the helpers for a project that end with prefixes `_path`, `_url` etc., but there is no documentation on what the difference is between `_path/2` and `_path/3`. Or, to be more precise, what option does it accept as the 3rd argument?

Macros can generate functions with documentation. For example:
